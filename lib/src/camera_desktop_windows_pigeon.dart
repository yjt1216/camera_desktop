...
/// Windows [CameraPlatform] via Pigeon (aligned with `camera_recorder` /
/// `camera_windows` native registration).
class CameraDesktopWindowsPigeon extends CameraPlatform {
  /// Creates a new Windows [CameraPlatform] implementation instance.
  CameraDesktopWindowsPigeon({@visibleForTesting CameraApi? api})
      : _hostApi = api ?? CameraApi();

  /// Registers the Windows implementation of [CameraPlatform].
  static void registerWith() {
    CameraPlatform.instance = CameraDesktopWindowsPigeon();
  }

  /// Capability flags for desktop-specific UI (native Pigeon path does not
  /// invoke MethodChannel `getPlatformCapabilities` yet).
  ...
  /// Preview mirror control (Pigeon); not part of [CameraPlatform] 2.12.
  ...
  Future<void> startVideoCapturing(VideoCaptureOptions options) async {
    ...
    String? outputPath = CameraDesktopCaptureHints.consumeNextVideoRecordingPath(
      options.cameraId,
    );
    if (outputPath == null || outputPath.isEmpty) {
      // The stock `camera` package never passes a path into
      // [VideoCaptureOptions]; desktop apps set one via
      // [CameraDesktopCaptureHints.setNextVideoRecordingPath]. When unset,
      // pick a temp file so native RecordHandler never receives an empty path.
      final stamp = DateTime.now().millisecondsSinceEpoch;
      outputPath =
          '${Directory.systemTemp.path}${Platform.pathSeparator}'
          'camera_desktop_${options.cameraId}_$stamp.mp4';
    }
    await _hostApi.startVideoRecording(options.cameraId, outputPath);
  }
