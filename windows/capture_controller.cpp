...
HRESULT CaptureControllerImpl::CreateCaptureEngine() {
  ...
  // If enumeration or MFCreateDeviceSource fails (or there are no capture
  // devices), continue video-only so preview/recording still work — same as
  // enableAudio: false from Dart.
  if (media_settings_.enable_audio() && !audio_source_) {
    const HRESULT audio_hr = CreateDefaultAudioCaptureSource();
    if (FAILED(audio_hr) || !audio_source_) {
      audio_source_ = nullptr;
    }
  }
